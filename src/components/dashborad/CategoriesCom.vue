  ...
  <v-container class="pa-6">
    ...
    <v-row class="mb-6" align="center" justify="start">
      <v-btn color="primary" class="mr-4" elevation="2" icon>
        <router-link :to="{ name: 'catigoreAdd' }" style="color: white">
    ...
    <v-row dense>
      <v-col
        v-for="(pro, i) in catigoryiesDashboard"
        :key="i"
        cols="12"
        sm="6"
        md="4"
        ...
        <v-card class="pa-4" elevation="4">
          <v-row align="center">
            ...
            <v-col cols="8">
              ...
              <v-card-subtitle>Slug: {{ pro.slug }}</v-card-subtitle>
              <v-card-subtitle>pageId: {{ pro.page_id }}</v-card-subtitle>
              <v-card-subtitle
                >Products: {{ pro.product.length }}</v-card-subtitle
              >
              <v-card-text class="mt-2">{{ pro.description }}</v-card-text>
            </v-col>
          </v-row>
...
export default {
  ...
  computed: {
    ...mapState(mystore, ["catigoryiesDashboard", "domin"]),
  },
  methods: {
    ...mapActions(mystore, ["getCatigoryDash"]),
    async up(event) {
      ...
      try {
        const res = await axios.post(
          `${this.domin}dashboard/categories/import`,
          formData,
          {
            headers: {
        ...
        await this.getCatigoryDash(); // تحديث تلقائي بعد الرفع
      } catch (err) {
    ...
    async down() {
      ...
      try {
        const res = await axios.get(
          `${this.domin}dashboard/categories/export`,
          {
    ...
    async dl(id) {
      ...
      try {
        const res = await axios.delete(
          `${this.domin}dashboard/categorie/delete/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        console.log("تم حذف التصنيف:", res.data);
      } catch (err) {
        console.error(err.response?.data || err);
      }

      await this.getCatigoryDash(); // تحديث بعد الحذف
    },
  },
  async mounted() {
